...
__attribute__((used, section(".requests"))) static volatile struct limine_kernel_address_request kernel_address_request = {.id = LIMINE_KERNEL_ADDRESS_REQUEST, .revision = 0};
...
void olive_entry(void)
{
    ...
    if (kernel_address_request.response == NULL || kernel_address_request.response->virtual_base == 0 || kernel_address_request.response->physical_base == 0)
    {
        FATAL("Failed to get kernel address (virt and phys)\n");
        hcf();
    }

    if (vmm_init(kernel_address_request.response->virtual_base, kernel_address_request.response->physical_base) != 0)
    {
